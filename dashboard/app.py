...
def render_tab_content(active_tab):
    ...
    if df is None:
        return dbc.Alert(
            "Los datos no están disponibles. Por favor, recarga el archivo.",
            color="warning",
            className="m-4",
        )
    ...
    if active_tab == "tab-gr":
        return run_gr_analysis(df)
    if active_tab == "tab-log":
        return run_log_regression_analysis(df)
    if active_tab == "tab-lstm":
        return run_lstm_prediction(df)
    if active_tab == "tab-comparison":
        return render_model_comparison(df)
    if active_tab == "tab-density":
        return render_density_analysis(df)
    if active_tab == "tab-reports":
        return render_reports_module(df)
    return dbc.Alert("Pestaña no disponible.", color="warning", className="m-4")
...
def process_and_start(n_clicks, contents, filename):
    ...
    try:
        ...
        if preprocess_seismic_data(csv_path):
            return {'phase': 1}, dbc.Alert(
                f"✅ Dataset {filename} procesado correctamente. Iniciando dashboard...",
                color="success",
            )
